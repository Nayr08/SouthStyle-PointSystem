...
export default function ScanPage() {
  ...
  return (
    <>
      <main className="animate-page phone-shell bg-slate-950 pb-28 text-white">
        <section className="relative min-h-[calc(100svh-88px)] overflow-hidden bg-slate-950">
          ...
          <div className="relative z-10 flex min-h-[calc(100svh-88px)] flex-col px-5 pt-6">
            <div className="mb-8 flex items-center justify-between">
              <Link href="/" className="tap-button grid h-10 w-10 place-items-center rounded-full bg-white/15 backdrop-blur">
                <ArrowLeft size={20} />
              </Link>
              <h1 className="text-lg font-black">{isScanner ? 'Scan QR Code' : 'My QR Code'}</h1>
              <button
                onClick={() => setMode(isScanner ? 'account' : 'scan')}
                className="tap-button grid h-10 w-10 place-items-center rounded-full bg-white/15 backdrop-blur"
            ...
            {isScanner ? (
              <div className="flex flex-1 flex-col items-center justify-center pb-10">
                <div className="relative grid aspect-square w-full max-w-[290px] place-items-center">
                  ...
                  <div className="scanner-line absolute left-8 right-8 top-1/2 h-px bg-emerald-300 shadow-[0_0_18px_rgba(52,211,153,0.9)]" />
                ...
                <div className="mt-8 flex gap-3">
                  <button className="tap-button grid h-12 w-12 place-items-center rounded-full bg-white/15 backdrop-blur" title="Flash">
                    <Flashlight size={20} />
                  </button>
                  <button className="tap-button grid h-12 w-12 place-items-center rounded-full bg-white/15 backdrop-blur" title="Switch camera">
                    <RotateCcw size={20} />
                  </button>
                </div>
              </div>
              ...
              <div className="flex flex-1 flex-col items-center justify-center pb-10">
                <div className="animate-pop w-full max-w-[310px] rounded-[32px] border border-white/25 bg-white/15 p-5 text-center shadow-2xl shadow-black/25 backdrop-blur-xl">
