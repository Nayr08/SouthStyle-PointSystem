...
const PIN_LENGTH = 4;
const APP_VERSION_KEY = 'southstyle:app-version';
  ...
  useEffect(() => {
    ...
    const checkForAppUpdate = async () => {
      try {
        ...
        window.localStorage.setItem(APP_VERSION_KEY, version);
        toast('App updated. Refreshing now...', { icon: '🔄' });
        ...
        window.setTimeout(() => {
          ...
          nextUrl.searchParams.set('appv', version);
          window.location.replace(nextUrl.toString());
        }, 700);
      } catch {
        // Ignore update-check failures so login flow stays smooth.
      }
    };
